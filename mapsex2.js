...
////2a.  Create an array of only the names
/*
Solution
let names = characters.map(char=>char.name)
console.log(names)
*/
///2b.   Create an array that includes the name and mass of all charactors who mass is greater than 50.
/////////////2b.   Make sure you check the type of the mass.

//single line solution:  let charsAndMass = charecters.map(char=>parseInt(char.mass) > 50 ? {name:char.name, mass:char.mass} :null).filter(e=>)
/* Alternate Solution
const starNames = characters.map(function(char){
    return char.name;
});
...
//
//charsAndMass = characters.filter(char=>{
//    if(parseInt(char.mass) > 50){
//        return true
//    }
//    return false
//
//})
//console.log(charsAndMass.length)
//

//console.log['yes', 'no', true, false, 0].filter(e=>0)
///2c.   Create an array of objects of only the characters that have at  least one starship. 
/////    The objects only need 'name' and 'hasStarship' as its keys.
charAndStarship = characters.filter(function(char){
    if(parseInt(char.starships.length) > 0){
        return true
    }
    return
}) 

let finalOut = charAndStarship.map(function(char){
    return ({name:char.name, hasStarship:char.starships})
})
console.group(charAndStarship)

///2d.   Create an array of names of only the charecters who have both a starship and a vehicle. 

charVehStarship = characters.filter(function(char){
    if(parseInt(char.starships.length) > 0){
        return true
    }
    else if(parseInt(char.vehicles.length) > 0){
        return true
    }
    return
}) 

let finalOut = charVehStarship.map(function(char){
    return ({name:char.name, charVehStarship:char.starships. charVehStarship.vehicles})
})
console.group(charVehStarship)